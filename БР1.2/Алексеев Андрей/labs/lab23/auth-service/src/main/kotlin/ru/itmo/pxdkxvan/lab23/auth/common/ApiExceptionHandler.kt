...
class ApiExceptionHandler {
    ...
    private fun resolveStatus(message: String?): HttpStatus {
        ...
        return when {
            "invalid email or password" in normalized -> HttpStatus.UNAUTHORIZED
            "invalid refresh token" in normalized -> HttpStatus.UNAUTHORIZED
            "inactive" in normalized -> HttpStatus.FORBIDDEN
            "already in use" in normalized -> HttpStatus.CONFLICT
